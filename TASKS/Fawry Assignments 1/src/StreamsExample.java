...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        Consumer<Author> activaAuthors = new Consumer<Author>() {
            @Override
            public void accept(Author author) {
                if (author.active) {
                    System.out.println(author);
        ...
        authors.stream()
                ...
                .forEach(auther -> System.out.println(auther));


        banner("Active books for all authors");
        // TODO With functional interfaces declared
        Consumer<Author> activeBooks = new Consumer<Author>() {
            @Override
            public void accept(Author author) {
        ...
        // TODO With functional interfaces declared
        Consumer<Author> averagePrice = new Consumer<Author>() {
            @Override
            public void accept(Author author) {
                System.out.println(author.name);
                author.books.stream()
                        ...
                        .average()
                        .ifPresentOrElse(avg -> System.out.println("Average price: " + avg),() -> System.out.println("No Books"));
            }
        };
        authors.stream()
                .forEach(averagePrice);
        ...
        authors.stream()
                .forEach(author -> {;
                    ...
                    author.books.stream()
                            .mapToInt(book -> book.price)
                            .average()
                            .ifPresentOrElse(avg -> System.out.println("Average price: " + avg),() -> System.out.println("No Books")); // x author handle
                });

        banner("Active authors that have at least one published book");
        // TODO With functional interfaces declared
        Consumer<Author> actwithat1published= new Consumer<Author>() {
            @Override
            public void accept(Author author) {
